...
class RhodiumGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("🪙 Rhodium Wallet - SISTEMATO")
        self.root.geometry("900x700")
        self.root.configure(bg='#1a1a1a')
        
        print("🔄 Inizializzazione wallet...")
        self.wallet = RhodiumWallet()  # Già sincronizzato all'init
    ...
    def setup_ui(self):
        ...
        # Mining
        mining_frame = ttk.LabelFrame(main_frame, text="⛏️ Mining - DIFFICULTY 10", padding="10")
        ...
        sync_btn = ttk.Button(btn_frame, text="🔄 Sincronizza", command=self.force_sync)
        sync_btn.pack(side=tk.LEFT, padx=5)
        
        self.mining_log = scrolledtext.ScrolledText(mining_frame, height=8, bg='#1a1a1a', fg='white', font=('Courier', 9))
        ...
        ttk.Label(send_frame, text="A:").pack(side=tk.LEFT)
        ...
        ttk.Label(send_frame, text="Importo:").pack(side=tk.LEFT, padx=(20,0))
        ...
        # History
        history_label = ttk.Label(trans_frame, text="📜 Cronologia:", font=('Arial', 11, 'bold'))
    ...
    def force_sync(self):
        ...
        self.mining_log.insert(tk.END, "🔄 Sincronizzazione forzata...\\n")
    ...
    def update_display(self):
        ...
        try:
            # Sincronizza wallet
            self.wallet.sync_completo()
            
            # Aggiorna UI
            balance = self.wallet.get_balance()
            self.balance_label.config(text=f"💰 Saldo: {balance:,.2f} RHO")
    ...
    def start_mining(self):
        ...
        if not self.mining_thread or not self.mining_thread.is_alive():
            ...
            self.mining_log.insert(tk.END, "⛏️ Mining avviato (Difficulty 10)\\n")
            self.mining_log.see(tk.END)
...
if __name__ == "__main__":
    print("🖥️ Avvio Rhodium Wallet - VERSIONE SISTEMATA")
    main()
