...
class RhodiumGUI:
    ...
    def setup_ui(self):
        ...
        ttk.Button(actions_frame, text="➕ Crea Transazione Test", 
                  command=self.create_test_transaction).grid(row=0, column=3, padx=5)
    ...
    def auto_update_balance(self):
        self.update_balance()
        self.root.after(30000, self.auto_update_balance)
    ...
    def create_test_transaction(self):
        """Crea una transazione di test per il mining"""
        try:
            # Crea un indirizzo di test
            test_wallet = RhodiumWallet("test_wallet.dat")
            test_address = test_wallet.get_address()
            
            # Crea transazione verso l'indirizzo di test
            tx = Transaction(self.wallet.get_address(), test_address, 10.0, 0.001)
            if self.blockchain.add_transaction(tx):
                self.mining_log.insert(tk.END, f"✅ Transazione test creata: 10 RHO a {test_address}\n")
                self.mining_log.see(tk.END)
                messagebox.showinfo("Successo", f"Transazione test creata!\n10 RHO a {test_address}")
            else:
                messagebox.showerror("Errore", "Errore creazione transazione test")
        except Exception as e:
    ...
    def send_transaction(self):
        ...
        try:
            ...
            if self.blockchain.add_transaction(tx):
                ...
                self.update_balance()
                self.mining_log.insert(tk.END, f"✅ Nuova transazione: {amount} RHO a {recipient}\n")
                self.mining_log.see(tk.END)
            else:
    ...
    def mining_worker(self):
        while self.mining_running:
            try:
                # Mine one block - ORA FUNZIONA ANCHE SENZA TRANSAZIONI
                ...
                if block:
                    ...
                    self.root.after(0, self.mining_log.insert, tk.END, "❌ Errore: mining fallito\n")
    ...
    def on_block_mined(self, block):
        ...
        self.mining_log.insert(tk.END, f"   📦 Transazioni: {len(block.transactions)}\n")
        self.mining_log.see(tk.END)
        self.update_balance()
        self.load_transaction_history()
